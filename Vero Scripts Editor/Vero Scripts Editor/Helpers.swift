...
extension String {
    ...
    public func enumeratePattern(using regex: any StringProtocol) -> [Range<String.Index>] {
        var matches = [Range<String.Index>]()
        var range = self.startIndex..<self.endIndex
        repeat {
            if let matchRange = self.range(of: regex, options: .regularExpression, range: range) {
                matches.append(matchRange)
                range = matchRange.upperBound..<self.endIndex
            } else {
                range = self.endIndex..<self.endIndex
            }
        } while (range.lowerBound < self.endIndex)
